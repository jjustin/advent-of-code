...
fn enumerate_hand(hand: &str) -> i32 {
    ...
    if max_card != 'J' {
        let j_count = count_by_card.entry('J').or_default().clone();
        *count_by_card.entry(max_card.clone()).or_default() += j_count;

        count_by_card.remove(&'J');
    }
